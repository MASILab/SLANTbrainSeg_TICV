...
class Trainer(object):
    ...
    def __init__(self, cuda, model, optimizer=None,
                 test_loader=None, lmk_num=None,
                 train_root_dir=None, out=None, outmodel=None, max_epoch=None, batch_size=None,
    ...
    def validate(self):
        ...
        results_epoch_dir = osp.join(out, 'epoch_0034')
    ...
    def train_epoch(self):
        for epoch in tqdm.trange(self.epoch, self.max_epoch,
            ...
            if self.finetune:
                old_model_pth = '%s/135model_epoch_ft_%04d.pth' % (out, self.epoch)
                self.model.load_state_dict(torch.load(old_model_pth))
                
                self.train()
                self.validate()
                new_model_pth = '%s/135model_epoch_ft_%04d.pth' % (out, self.epoch+1)
                torch.save(self.model.state_dict(), new_model_pth)
    ...
    def test_epoch(self):
       ...
       #  train_root_dir = osp.join(self.out, 'models')
         ...
         train_root_dir = self.outmodel
         #train_root_dir = osp.join(self.out, 'models')
         model_pth = '%s/135model_epoch_ft_0034.pth' % (train_root_dir)
         #model_pth = '%s/model_epoch_0004.pth' % (train_root_dir)
         if os.path.exists(model_pth):
