...
[ll s_inds] = process_target(atlas_model_fname, res_corr_fname, res_norm_fname, mdir,target_bname);
...
function [likelihood s_inds] = process_target(atlas_model_fname, res_corr_fname, res_norm_fname, mdir,target_bname)
    ...
    result_dir = [mdir filesep 'FinalResult'];
    mkdir(result_dir);
    system(sprintf('cp %s %s\n', res_norm_fname, result_dir));
    result_fname = sprintf('%s/target_processed.nii.gz', result_dir);
    final_fname = sprintf('%s/%s.nii.gz', result_dir,target_bname);
    system(sprintf('cp %s %s\n', result_fname, final_fname));
    system(sprintf('rm %s\n', result_fname));
